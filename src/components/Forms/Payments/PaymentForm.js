...
const PaymentForm = ({
  ...
  const [haveFile, setHaveFile] = useState(editMode)
  ...
  // from edit view
  const [keepInitialData, setKeepInitialData] = useState(editMode)
    ...
    */
    ...
    if (!keepInitialData && datesExists && startBeforeEnd && endBeforeToday) {
      ...
      fetchRevenue = setTimeout(async () => {
        try {
          const url = `/${applicationId}/insights/revenue/daily?period.gte=${startDate}&period.lte=${endDate}`

          let response = await apiCallApplications(url, 'GET')
          if (response) {
            setValue('income', response.summary_current.toFixed(2))
          }
        } catch (e) {
          console.error('ERROR on GET income data: ', e)
        } finally {
