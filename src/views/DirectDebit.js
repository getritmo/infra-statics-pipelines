...
export const DirectDebit = (props) => {
  ...
  const stripeId =
    application.currency === 'GBP'
      ? process.env.REACT_APP_STRIPE_SECRET_ID_UK
      : process.env.REACT_APP_STRIPE_SECRET_ID
  const stripePromise = loadStripe(stripeId)

  const isError = props.match.params.any === 'cancel'
  ...
  const handleReturn = async (checkoutResponse) => {
    const { session_id, nonce } = Object.fromEntries(
      new URLSearchParams(window.location.search),
    )

    if (session_id && nonce) {
      try {
        const url = `/${application.application_id}/direct-debit/checkout/${checkoutResponse}`
        const body = {
          session_id: session_id,
          nonce: nonce,
        }

        let response = await apiCallApplications(url, 'POST', body)
        updateStatus(response)
      } catch (e) {
        console.error('ERROR on POST direct debit')
      }
    } else {
      console.error('ERROR missing success parameters, session_id and/or nonce')
    }
  }

  const isPageResult = props.match.params.any
  const isPageResultSucess = props.match.params.any === 'success'

  const last4digits =
    application?.payments_methods?.[0].last_four_digits || false
  const mandate_reference =
    application?.payments_methods?.[0].mandate_reference || false

  if (application) {
    if (preventMultiple) {
      if (isPageResult) {
        preventMultiple = false
        handleReturn(props.match.params.any)
      }
    }
  }
  ...
  const handleClick = async () => {
    ...
    setIsLoading(true)
    try {
      const url = `/${application.application_id}/direct-debit/checkout`
      let response = apiCallApplications(url, 'GET')
      await stripe.redirectToCheckout({
        sessionId: response.id,
      })
    } catch (e) {
      console.error('ERROR on GET direct debit checkout: ', e)
    }
  }
  ...
  return (
    <>
      <Container className="">
        <>
          <Breadcrumb
            previous={translate('views.direct_debit.breadcrumb.previous')}
            actual={translate('views.direct_debit.breadcrumb.current')}
          />
          <>
          ...
          <div className="layout-app__info">
            {application && (
              <>
                {!inArray(decode(application.status), 9) && (
                  <>
                    <h2 className="layout-app__title--h2">
                      <div className="alert">
                        <StatusIcon status="alert" />
                        {translate('views.direct_debit.text2')}
                      </div>
                    </h2>
                    <p>{translate('views.direct_debit.text3')}</p>

                    <Validator status={application} actual={10} />
                  </>
                )}

                {/* Una vez verificada la propiedad de la empresa */}
                {inArray(decode(application.status), 9) &&
                  !inArray(decode(application.status), 10) &&
                  !isPageResultSucess && (
                    <>
                      <h2 className="layout-app__title--h2">
                        <span className="pending">
                          <StatusIcon status="pending" />
                          {translate('views.direct_debit.text4')}
                        </span>
                      </h2>
                      {isError && (
                        <>
                          <p>{translate('views.direct_debit.text5')}</p>

                          <p>{translate('views.direct_debit.text6')}</p>
                        </>
                      )}
                      {!isError && (
                        <p>{translate('views.direct_debit.text7')}</p>
                      )}

                      <br />
                      <Button
                        className={'btn btn-secondary ' + isLoading}
                        onClick={handleClick}
                      >
                        <span>
                          <span>{translate('views.direct_debit.text8')}</span>
                          <div className="min-logo">
                            <img
                              src="/images/svg/ritmo-logo-min-circle-white.svg"
                              alt=""
                              width="20"
                              height="20"
                      ...
                      <p>
                        {translate('views.direct_debit.text9')}{' '}
                        <a
                          href="mailto:operations@getritmo.com"
                          rel="noopener noreferrer"
                          target="_blank"
                          className="link link__full"
                        >
                          operations@getritmo.com
                        </a>
                        .
                      </p>
                    </>
                ...
                {/* Una vez autorizado el direct debit */}
                {inArray(decode(application.status), 10) && (
                  <>
                    <h2 className="layout-app__title--h2">
                      <span className="accepted">
                        <StatusIcon status="accepted" />
                        {translate('views.direct_debit.text10')}
                      </span>
                    </h2>
                    <p>{translate('views.direct_debit.text11')}</p>
                    {last4digits && (
                      <p>
                        {translate('views.direct_debit.text12', {
                    ...
                    {mandate_reference && (
                      <p>
                        {translate('views.direct_debit.text13', {
                    ...
                    <p>
                      {translate('views.direct_debit.text14')}{' '}
                      <a
                        href="mailto:operations@getritmo.com"
                        target="_blank"
                        rel="noopener noreferrer"
...
export default withAuthenticationRequired(DirectDebit, {
  loginOptions: {
